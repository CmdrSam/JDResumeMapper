...
def _error_summary_row(
    *,
    ...
    return {
        ...
        "recruiter_notes": recruiter_notes.strip()[:_MAX_RECRUITER_NOTES_CHARS],
    }
...
def process_match_job(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    resume_order: list[tuple[Path, str, str, str]] = []
    for item in resume_items:
        ...
        notes_raw = str(item.get("recruiter_notes") or "").strip()
        notes = notes_raw[:_MAX_RECRUITER_NOTES_CHARS]
        resume_order.append((rpath, display_name, h, notes))
    ...
    for rpath, display_name, h, notes in resume_order:
        if h in failed_hash_to_reason:
            ...
            continue
        try:
            base = content_hash_to_candidate[h]
            candidate = dict(base)
            if notes:
    ...
    for t in resume_order:
        rpath, display_name, h, notes = t
        if h not in content_hash_to_candidate:
            continue
        base = content_hash_to_candidate[h]
        cand = dict(base)
        if notes:
            cand["recruiter_notes"] = notes
        candidate_by_resume_key[str(rpath.resolve())] = cand
